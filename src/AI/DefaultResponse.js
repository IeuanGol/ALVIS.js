...
class DefaultResponse {
  ...
  defaultHandler(message, response) {
    ...
    if (response.result.fulfillment.speech !== ""){
      ...
      message.reply("I am unable to fulfill your `" + response.result.action + "` query at this time. Perhaps this feature is still in development.")
  ...
  disabledServiceHandler(message, response, service) {
    var embed = new Discord.RichEmbed();
    embed.setColor(parseInt(this.bot.colours.bot_embed_colour));
    embed.setThumbnail(this.bot.webAssets.packetcloud_icon);
    embed.setDescription("Services can be enabled by providing required API or service credentials in my configuration files.\n\nCheck my documentation, or contact my developers for more information:\n[Documentation](" + this.bot.webAssets.alvis_github_readme + ")\n[GitHub](" + this.bot.webAssets.alvis_github + ")\n[Website](" + this.bot.webAssets.packetcloud_website + ")")
    embed.setURL(this.bot.webAssets.packetcloud_website);
    var discord_bot = this.bot;
    if (message.channel instanceof Discord.TextChannel) discord_bot.messageCleanupQueue.add(message, 1, true);
    message.reply("My **" + service + "** service module is not currently active. I am unable to respond to your request.", {"embed": embed})
    .then((msg) => {if (msg.channel instanceof Discord.TextChannel) discord_bot.messageCleanupQueue.add(msg, 1, true)});
